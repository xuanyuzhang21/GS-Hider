	...
	float* collected_semantic_feature) 
{
	...
	__shared__ float collected_s_feature[NUM_SEMANTIC_CHANNELS * BLOCK_SIZE];
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			for (int i = 0; i < NUM_SEMANTIC_CHANNELS; i++) 
				collected_s_feature[i * BLOCK_SIZE + block.thread_rank()] = semantic_feature[coll_id * NUM_SEMANTIC_CHANNELS + i];
		}
		block.sync();
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			{
				const float f = collected_s_feature[ch * BLOCK_SIZE + j];
			...
			for (int i = 0; i < NUM_SEMANTIC_CHANNELS; i++)
				bg_dot_dpixel += bg_color[0] * dL_dfeaturepixel[i];
